    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while (playAgain)
            {
                ...
                while (true)// Loop for ensuring a valid positive integer for height
                {
                    Console.WriteLine($"Ange höjden på pyramiden."); // Ask the player to choose the height of the pyramid.
                    string input = Console.ReadLine();
                    if (int.TryParse(input, out height) && height > 0) // Check if the input is a positive integer
                    {
                    ...
                    {
                        attempts++;
                        Console.WriteLine("Fel. Ange ett positivt heltal."); // Invalid input message

                        if (attempts >= maxAttempts) // If you dont enter a valid number in your tries, your height will get the standard height.
                        {
                            Console.WriteLine($"För många försök använda, höjden sätts till standardhöjden {defaultHeight}.");
                ...
                if (choice.ToLower() == "o") // Here we have the code for the inverted part.
                {
                    for (int i = height; i >= 1; i--) // Start on the height, and then reverse down to 1.
                    {
                        ...
                        Console.WriteLine(new string(symbol, i * 2 - 1));// Print out the pyramid's row with the right amount of symbols.
                    }
                }
                for (int i = 1; i <= height; i++) // Start on 1, and go down to the set height.
                {
                    if (choice.ToLower() == "f") // The color part.
                    {
                        ...
                        Console.WriteLine(new string(symbol, i * 2 - 1));// Making the pyramid's shape.
                        Console.ResetColor();
                    }
